...
class ServiceRequest(Document):
    ...
    def set_customer_and_project(self):
        if getattr(self, "contract", None):
            ...
            if frappe.db.has_column("Project", "contract"):
                self.erpnext_project = frappe.db.get_value(
                    "Project", {"contract": contract.name}, "name"
                )
            return
    ...
    def validate_workflow_transitions(self):
        ...
        old_status = (
            frappe.db.get_value("ServiceRequest", self.name, "status")
            if not self.is_new()
            else None
        )
        ...
        if (
            old_status == "Open"
            and self.status == "In Progress"
            and not self.assigned_to
        ):
            frappe.throw(
                _("Cannot set status to 'In Progress' without assigning an engineer.")
            )
        elif (
            old_status == "In Progress"
            and self.status == "Completed"
            and not self.linked_report
        ):
            frappe.throw(
                _("Cannot set status to 'Completed' without linking a Service Report.")
            )
        elif (
    ...
    def calculate_sla_deadline(self):
        ...
        if self.type == "Emergency" and self.priority == "High":
            self.sla_deadline = add_hours(
                self.creation, 4
            )  # 4 hours for high emergency
        elif self.type == "Emergency" and self.priority == "Medium":
            self.sla_deadline = add_hours(
                self.creation, 8
            )  # 8 hours for medium emergency
    ...
    def check_sla_breach(self):
        if (
            ...
            telegram_chat_id = (
                123456789  # REPLACE WITH ACTUAL CHAT ID OF ADMIN/DEPT HEAD
            )
            ...
            try:
                ...
                response = requests.post(
                    fastapi_backend_url, headers=headers, json=payload
                )
                response.raise_for_status()  # Raise HTTPError for bad responses (4xx or 5xx)
                frappe.log_by_page(
                    f"SLA breach notification sent to Telegram for {self.name}",
                    "SLA Notification",
                )
            except requests.exceptions.RequestException as e:
                frappe.log_error(
                    f"Failed to send SLA breach Telegram notification for {self.name}: {e}",
...
System"""
            ...
            try:
                frappe.sendmail(
                    recipients=recipient_email, subject=subject, message=body
                )
                frappe.log_by_page(
                    f"SLA breach email notification sent for {self.name}",
                    "SLA Notification",
                )
            except Exception as e:
                frappe.log_error(
                    f"Failed to send SLA breach email notification for {self.name}: {e}",
                    "SLA Notification Error",
                )
