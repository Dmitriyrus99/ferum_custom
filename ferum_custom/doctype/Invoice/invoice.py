...
try:
    ...
    frappe.log_error(
        "gspread library not found. Google Sheets integration will be disabled.",
...
class Invoice(Document):
    ...
    def set_customer_and_project(self):
        ...
        if getattr(self, "contract", None):
            ...
            if (
                hasattr(self, "counterparty_type")
                and self.counterparty_type == "Customer"
            ):
    ...
    def validate_status_transitions(self):
        old_status = (
            frappe.db.get_value("Invoice", self.name, "status")
            if not self.is_new()
            else None
        )
    ...
    def sync_to_google_sheets(self):
        if not GOOGLE_SHEETS_INTEGRATION_ENABLED:
            frappe.msgprint(
                _("Google Sheets integration is disabled. gspread library not found.")
            )
            return
        ...
        try:
            ...
            spreadsheet = gc.open(
                "Ferum Invoices Tracker"
            )  # Or gc.open_by_key("YOUR_SPREADSHEET_ID")
            worksheet = spreadsheet.worksheet(
                "Sheet1"
            )  # Or your specific worksheet name
            ...
            row_data = [
                ...
                getattr(self, "erpnext_project", None)
                or self.project
                or "",  # Linked Project (new/legacy)
            ...
            if cell:
                # Update existing row
                worksheet.update(
                    f"A{cell.row}:J{cell.row}", [row_data]
                )  # Adjust range (A:J) as per your columns
            ...
            frappe.log_error(
                f"Google Sheets sync failed for Invoice {self.name}: {e}",
