...
def _ensure_customer(link_value: str, *, company: str | None = None) -> str:
    ...
    customer_group = frappe.db.get_value("Customer Group", {"name": ["like", "%All%"]}, "name") or frappe.db.get_value(
        "Customer Group", {}, "name"
    )
...
def _get_or_create_contract_for_service_project(sp_name: str) -> str | None:
    ...
    if frappe.db.has_column("Contract", "contract_year"):
        contract.contract_year = (contract.start_date.year if contract.start_date else frappe.utils.now_datetime().year)
...
def _backfill_refs(service_project_name: str, contract_name: str, erp_project: str | None) -> None:
    # Service Request
    if _exists("Service Request") and frappe.db.has_column("Service Request", "project") and frappe.db.has_column(
    ...
    if _exists("Service Maintenance Schedule") and frappe.db.has_column(
        "Service Maintenance Schedule", "service_project"
    ...
    # Invoice (custom)
    if _exists("Invoice") and frappe.db.has_column("Invoice", "project") and frappe.db.has_column("Invoice", "contract"):
        frappe.db.sql(
            """
            update `tabInvoice`
            set contract = %s
