...
function ferum_render_project_documents(frm) {
  ...
  if (uploadBtn) {
    uploadBtn.on('click', () => {
      const defaultContract = frm.doc && frm.doc.contract ? frm.doc.contract : '';
      const d = new frappe.ui.Dialog({
        ...
        fields: [
          ...
          {fieldname: 'contract', label: 'Контракт', fieldtype: 'Link', options: 'Contract', default: defaultContract},
    ...
    uploadBtn.on('click', () => {
      const d = new frappe.ui.Dialog({
        ...
        primary_action(values) {
          ...
          const fd = new FormData();
          fd.append('file', file, file.name);
          fd.append('method', 'ferum_custom.api.project_documents.upload_contract_document');
          fd.append('contract', frm.doc.name);
          fd.append('ferum_doc_title', values.doc_title);
          fd.append('ferum_doc_type', values.doc_type);
