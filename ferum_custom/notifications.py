...
def _chat_ids_for_project_subscribers_typed(project: str, *, parenttype: str) -> set[int]:
    if not project:
        return set()
    if not frappe.db.exists("DocType", "Project Telegram User Item") or not frappe.db.exists("DocType", "Telegram User Link"):
        return set()
...
def _service_request_context(doc) -> dict:
    ...
    address = None
    site_engineer = None
    if project_site and frappe.db.exists("DocType", "Project Site") and frappe.db.exists("Project Site", project_site):
...
def notify_service_request_status_change(doc, method):
    ...
    try:
        ...
        if getattr(before, "assigned_to", None) != getattr(doc, "assigned_to", None):
            changes.append(f"Назначено: {getattr(before, 'assigned_to', None)} → {getattr(doc, 'assigned_to', None)}")
        if getattr(before, "priority", None) != getattr(doc, "priority", None):
            changes.append(f"Приоритет: {getattr(before, 'priority', None)} → {getattr(doc, 'priority', None)}")
        if not changes:
            return
