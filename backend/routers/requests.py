...
frappe_client = FrappeClient(
    settings.ERP_API_URL, settings.ERP_API_KEY, settings.ERP_API_SECRET
)
...
async def get_request(
    request_name: str, current_user: dict = Depends(get_current_user)
):
    user_roles = current_user.get("roles", [])
    user_name = current_user.get("name")
...
async def create_request(
    ...
    current_user: str = Depends(
        has_role(["Project Manager", "Administrator", "Office Manager", "Client"])
    ),
):
    try:
...
async def upload_request_attachment(
    ...
    current_user: str = Depends(
        has_role(["Project Manager", "Administrator", "Engineer", "Office Manager"])
    ),
):
    try:
        ...
        photos = list(getattr(service_request_doc, "photos", []) or [])
        photos.append(
            {"photo": new_attachment.name}
        )  # Assuming 'photo' field in child table links to CustomAttachment name

        frappe_client.set_value(
            "ServiceRequest", service_request_doc.name, {"photos": photos}
        )
