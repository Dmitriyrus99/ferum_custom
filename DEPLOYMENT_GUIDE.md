...
PDF/print rendering relies on `wkhtmltopdf 0.12.x (with patched qt)`. After changing the Dockerfile, rebuild the image and restart your stack. For a non-Docker bench install, install wkhtmltopdf at the OS level and restart bench processes (e.g. `bench restart` / supervisor/systemd).
